function [ glycans ] = Compartmentalization3( Trials,ResidenceTime, ENZ,inputSub,tip )
...
for i=1:Trials
    ...
    [GlycanSim{1},Ends{1},Times(1),NoReactions(1)]=OneCompartmentSim(ResidenceTime(1),LINKAGES{1},inputSub,tip,0);
    if ~isempty(GlycanSim{1})
        for j=2:noComp
            [GlycanSim{j},Ends{j},Times(j),NoReactions(j)]=OneCompartmentSim(ResidenceTime(j),LINKAGES{j},GlycanSim{j-1},Ends{j-1},0);
        end
    else
        for j=2:noComp
