...
for i=3:68
    ...
    abundance_matrix_1{1,i}=no;
    for j=1:174
        ix=find(ismember(Freq(:,1),high_abund{j,1}));
        if isempty(ix)
            abundance_matrix_1{j+1,i}=0;
        else
            f=Freq(ix,2);
            abundance_matrix_1{j+1,i}=cell2mat(f);
        end
    end
end
...
for i=3:68
    ...
    no=str2num(fileName(11:end-4));
    abundance_matrix_2{1,i}=no;
    for j=1:174
        ix=find(ismember(Freq(:,1),high_abund{j,1}));
        if isempty(ix)
            abundance_matrix_2{j+1,i}=0;
        else
            f=Freq(ix,2);
            abundance_matrix_2{j+1,i}=cell2mat(f);
        end
    end
end
