function [ glycan,ends,time,k ] = OneCompartmentSimER( ResidenceTime,LinkageMap, glycan,ends,Init_Time, compartments )
...
if isempty(glycan)
    %if FirstComp==1
        ER=exprnd(ResidenceTime,compartments,1);
        WT1=[sum(ER),exprnd(1)];
        if WT1(2)>WT1(1)
            time=time+WT1(1);
            return
...
while export==0
    ...
    if isempty(UniqueActiveEnzymes) %If no enzymes can act
        export=export+1;
        time=time+sum(exprnd(ResidenceTime,3,1));
        break
    else
        WT=[exprnd(1),sum(exprnd(ResidenceTime,3,1))];
    end
